...
$(document).ready(function(){
    ...
    */
    ...
    var rotateFunc = function (item, tip,count,post){
    ...
    window.addEventListener('message',(event)=>{
       event.ports[0].onmessage =(result)=>{
         if(result.data === 'start')
         {
            if(turnWheel.bRotate) return;
                ...
                var item = randomNum(0,count - 1);
                // 开始抽奖
                rotateFunc(item, turnWheel.rewardNames[item],count, event.ports[0]);
         }
       }
    });
